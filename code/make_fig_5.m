%make_fig_4
%Content: validaytion of tensorial MGE
close all
...
Kiso_vec = zeros(n_sub, nk);
Kiso_err = zeros(n_sub, nk);
Kiso_true = zeros(n_sub, nk);
Kaniso_vec = zeros(n_sub, nk);
Kaniso_err = zeros(n_sub, nk);
Kaniso_true = zeros(n_sub, nk);
Kinf_iso_vec = zeros(n_sub, nk);
Kinf_iso_err = zeros(n_sub, nk);
Kinf_iso_true = zeros(n_sub, nk);
Kinf_aniso_vec = zeros(n_sub, nk);
Kinf_aniso_err = zeros(n_sub, nk);
Kinf_aniso_true = zeros(n_sub, nk);
k_vec = zeros(n_sub, nk);
k_err = zeros(n_sub, nk);
k_true = zeros(n_sub, nk);
...
for c = 1:numel(substrate_names)
    ...
    load(base_fn + "\fit_results\tensorial_MGE_fit_" + substrate_name, "MD", "Kiso", "Kaniso", "k", "Kinf_iso", "Kinf_aniso", "N_samples", "sn_r", "true_k", ...
...
grid(ax5, 'off')
% set(ax5, 'ytick', 0:20:100)
set(axs, 'box', 'off', 'tickdir', 'out')
ylim([ax1 ax2 ax3 ax4], [-0.4 1.2])
set([ax1 ax2 ax3 ax4], 'ytick', -0.4:0.4:1.2, 'xtick', 0:20:100)
grid(axs, 'minor')
axis(axs, 'square')
set(ax5, 'xtick', 0:20:100)
xlim(axs, [5 105])
% set(axs, 'box', 'off')
% beautify_axes(axs)
